...
| Stored procedures | `src/db/procedures.sql` (`sp_paginated_orders`, `sp_user_order_stats`) |
| Query optimization + EXPLAIN | `src/db/002_add_indexes.sql`, `docs/mysql-optimization.md` |
| Transactions | `POST /api/orders` (BEGIN/COMMIT/ROLLBACK) |
| GraphQL + N+1 fix | `src/graphql/schema.ts` (Apollo Server 4 + DataLoader) |
